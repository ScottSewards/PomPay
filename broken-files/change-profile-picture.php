...
$error = '';
$uploadDirectory = "images/profile-pictures"; //DIRECTORY FOR IMAGES
...
if(isset($_POST["upload"])) {
	...
	if(strlen($error) == 0) {
      ...
      if($width > $maxLargeWidth) { //SCALE IMAGE
		...
		header("location:change-profile-picture.php?c=confirm");
		//exit();
	}
}
...
<main>
  ...
  <section>
    ...
    if(strlen($largePhotoExists) > 0) {
      ...
      $(window).load(function () {
      	$('#thumbnail').imgAreaSelect({ aspectRatio: '1:<?php echo $thumbHeight / $maxThumbWidth;?>', onSelectChange: preview });
      });
      </script>
    <?php } ?>
    <?php
    ...
    if(strlen($largePhotoExists) > 0 && strlen($thumbPhotoExists) > 0){
    	...
    	if(strlen($largePhotoExists) > 0) {?>
    		...
    		<div align="center">
    			...
    			<div style="border:1px #e5e5e5 solid; float:left; position:relative; overflow:hidden; width:<?php echo $maxThumbWidth;?>px; height:<?php echo $thumbHeight;?>px;">
  		...
  		if($deleteImgOwnersID == $userIDCookie) {
    		...
    		//CHECK IF THE IMAGE IS NOT THE CURRENT SET IMAGE
  			if($deleteImagePath !== $currentBannerIMG) {
  				...
  				header("location: change-profile-picture.php"); //REFRESH PAGE
			  } else {
				//DISPLAY ERROR MESSAGE
				header("location: change-profile-picture.php?error2=1");
			}
		} else { // NOT THE USERS IMAGE
			header("location: change-profile-picture.php");
		}
	}
  ...
  while($queryArray = mysqli_fetch_array($oldProfileQuery)) {
    $pictureID = $queryArray['id'];
    $pictureOwnerID = $queryArray['owners_id'];
    $pictureLocation = $queryArray['location'];
    echo "<div style='float: left; border: solid 0px red;margin-top: 5px; margin-bottom: 5px;' >
		  <div style='; border: solid 0px black'>
        <a href='change-profile-picture.php?img=$pictureID'>
				<img src='$pictureLocation' alt='broken-link'/>
			  </a>
			</div>
			<div style='; border: solid 0px black'>
				<a href='change-profile-picture.php?d=$pictureID'>Delete</a>
			</div>
		</div>";
  }
  ?>
