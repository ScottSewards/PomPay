...
if ((isset($newAddress))	AND	(isset($confirmNewAddress))	AND	(isset($password))) {
	if ($newAddress==$confirmNewAddress) {
		...
		if(password_verify($password,$usersPassword)) {
			...
			mysqli_query($con, "UPDATE users SET bitcoin_address = '$newAddress' WHERE id = '$usersIDCookie;' ");
			header("location: change-bitcoin-address.php?s=updated");
		} else {
...
<main>
	...
	<section>
		...
		<form action="dashboard/change-bitcoin-address.php" method="post">
			<div class="fields">
				<div class="field">
					<label for="email">New Bitcoin Address</label>
					<input name="newAddress" placeholder="1FVtAuaz56xHUXrCNLbtMDxcw6o5GNn4xqX" required/>
				</div>
				<div class="field">
					<label for="email">Confirm Bitcoin Address</label>
        ...
        <div class="field">
			...
			<button type="submit" name="submit">Change Bitcoin Address</button>
		</form>
